...
it('requests follow-up defaults and records sent status without a send endpoint', async () => {
  const fetchMock = vi.fn().mockImplementation(() => Promise.resolve(new Response('{}')))
  vi.stubGlobal('fetch', fetchMock)
  await createFollowUp('app', { due_at: null, template_reference: null, channel: 'EMAIL' })
  await updateFollowUp('app', 'followup', { status: 'SENT' })
  expect(fetchMock).toHaveBeenNthCalledWith(1, '/api/applications/app/followups', expect.objectContaining({ method: 'POST', body: '{"due_at":null,"template_reference":null,"channel":"EMAIL"}' }))
